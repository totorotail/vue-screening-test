...
<template>
    <div class="flex flex-col items-center min-h-screen bg-gray-50 w-full" v-if="hospitalInfo">
        ...
        <PatientListContent :patients="allPatients" :filteredPatients="allPatients"
            :paginatedPatients="paginatedPatients" :searchQuery="searchQuery" :currentPage="currentPage"
            :itemsPerPage="itemsPerPage" :getGender="getGender" :getDaysAgo="getDaysAgo"
            @register="goToPatientRegistration" @select="goToPatientDetail" />

        <!-- 페이지네이션 -->
        <div v-if="allPatients.length > 0" class="min-h-[44px] flex justify-center items-center pt-2 space-x-6">
            <!-- 이전 화살표 버튼 -->
