...
<template>
    <div class="min-h-screen bg-gray-100 flex flex-col items-center">
        ...
        <WideLogo class="w-[90%] max-w-[1400px] mt-6 mb-6" :showSearch="true" userPlan="BASIC"
        ...
        <div class="w-[90%] max-w-[1400px] flex mt-6 space-x-6">
            ...
            <div class="w-1/3 bg-white shadow-lg rounded-lg p-6">
                ...
                <p><strong class="text-gray-600">이메일:</strong> {{ userInfo.email }}</p>
                <p><strong class="text-gray-600">병원이름:</strong> {{ userInfo.hospitalName }}</p>
                <p><strong class="text-gray-600">지역:</strong> {{ userInfo.location }}</p>
                <p><strong class="text-gray-600">플랜:</strong> {{ userInfo.plan }}</p>
                <div class="mt-4">
                    <button class="bg-gray-200 p-2 rounded-full">⚙️</button>
                </div>
            </div>
            ...
            <div class="w-2/3 bg-white shadow-lg rounded-lg p-6">
                ...
                <div class="flex space-x-4">
                    <div v-for="plan in availablePlans" :key="plan.name" class="flex-1 p-4 border rounded-lg shadow-lg"
                        ...
                        <button v-if="userInfo.plan === plan.name"
                            class="mt-2 w-full py-2 border rounded-lg bg-gray-200 cursor-not-allowed">
                            {{ plan.name }} 플랜중
                        </button>

                        <button v-else @click="changePlan(plan.name)"
                            class="mt-2 w-full py-2 border rounded-lg bg-blue-500 text-white hover:bg-blue-600">
                            {{ plan.name }} 플랜변경
                        </button>
