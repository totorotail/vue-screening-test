...
const saveHospitalInfo = () => {
    if (password.value === confirmPassword.value && password.value === currentUserPassword.value) {
        isPasswordValid.value = true;
        emit("update", { hospitalName: newHospitalName.value, location: newLocation.value });
    } else {
        isPasswordValid.value = false;
        alert("비밀번호가 일치하지 않습니다."); // 사용자에게 경고 메시지 표시
    }
};
</script>
    ...
    <div v-if="show"
        ...
        <div class="flex justify-between mt-4">
            ...
            <button @click="saveHospitalInfo"
                ...
                :disabled="password !== confirmPassword || password !== currentUserPassword">
                CONFIRM
            </button>
        </div>
    </div>
